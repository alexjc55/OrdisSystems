-- =============================================================================
-- eDAHouse — Полная схема базы данных (актуальная версия)
-- Сгенерировано из рабочей базы, очищено от служебных объектов Drizzle
--
-- Порядок развёртывания на чистом сервере:
--   1. psql $DATABASE_URL -f schema_setup.sql    ← создаёт структуру таблиц
--   2. psql $DATABASE_URL -f seed_initial.sql    ← admin + настройки + тема
--   3. Запустить приложение
--
-- migration_branches.sql — только для ОБНОВЛЕНИЯ существующих установок,
-- которые были развёрнуты до добавления мультифилиальности.
-- При чистой установке НЕ нужен (ветки уже включены в эту схему).
-- =============================================================================
