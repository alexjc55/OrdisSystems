...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.delete("/api/themes/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (theme.isBuiltIn) {
        return res.status(400).json({ message: "Cannot delete built-in theme" });
      }
  ...
  app.post("/api/themes/activate", isAuthenticated, async (req: any, res) => {
    try {
      if (req.user?.role !== "admin") {
        return res.status(403).json({ message: "Forbidden" });
      }
      ...
      const theme = await storage.getThemeByName(themeName);
      
      if (!theme) {
        return res.status(404).json({ message: "Theme not found" });
      }
      
      const updatedSettings = await storage.setActiveTheme(themeName);
