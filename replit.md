...
- **November 2025 - VPS SEO Middleware Production Ready (COMPLETED)**: Successfully deployed and verified meta injection middleware on production VPS (https://edahouse.ordis.co.il/). System now correctly injects Restaurant and ItemList JSON-LD schemas when serving requests to search engine bots. **Critical configuration**: PM2 must run with `NODE_ENV=production` using ecosystem.config.cjs with fork mode. Middleware requires `Accept: text/html` header to trigger (all real search bots send this by default). Testing command: `curl -A "Googlebot" -H "Accept: text/html" http://localhost:5000/` shows successful injection. Logs confirm detection and schema injection working perfectly. Documentation updated in VPS-DEPLOY-GUIDE.md and QUICK-FIX-SEO.md with proper testing commands.
...
- **November 2025 - SEO Helmet Migration (COMPLETED)**: Migrated SEO meta tag management from direct DOM manipulation to react-helmet-async for better SSR compatibility and cleaner code. SEOHead.tsx completely refactored to use Helmet components, HelmetProvider added to App.tsx, all browser-only APIs (localStorage, window) now have SSR-safe checks. Created SSR infrastructure files (entry-server.tsx, bot-detection.ts, ssr-middleware.ts, meta-injection-middleware.ts) with bot-specific HTML injection middleware. **Production deployment**: Middleware successfully running on VPS with PM2, correctly serving enhanced HTML with JSON-LD schemas to search engine bots while maintaining SPA performance for users.
