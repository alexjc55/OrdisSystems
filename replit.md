...
- **November 2025 - White Screen Fix (Critical)**: Resolved critical white screen issue after PWA updates by implementing automatic page reload on service worker activation. Added `controllerchange` event listener in `usePWA.ts` that triggers immediate reload when new service worker takes control, preventing version mismatch between cached JavaScript and updated service worker. This eliminates the most common cause of white screens for users, especially on iOS devices. All 5 recommended anti-cache solutions now fully implemented: (1) skipWaiting + clients.claim ✅, (2) controllerchange auto-reload ✅, (3) versioned cache clearing ✅, (4) Vite bundle hashing ✅, (5) Express SPA fallback ✅.
...
- **SEO Optimization**: Comprehensive SEO implementation with react-helmet-async for dynamic meta tag management. Features include: static meta tags in index.html for initial bot indexing, dynamic meta tags via Helmet for SPA navigation, og:image/twitter:image with fallbacks, hreflang tags for 4 languages with query preservation, GEO metadata, Product/BreadcrumbList structured data. SSR infrastructure created (entry-server.tsx, bot-detection.ts) but not yet fully implemented due to Vite configuration constraints.
