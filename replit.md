...
**Данные правила ВСЕГДА должны соблюдаться при любых изменениях в проекте:**

1. **ЦВЕТОВЫЕ СТИЛИ:**
   - ❌ НИКОГДА не использовать жестко заданные цвета (bg-blue-600, text-red-500, etc.)
   - ✅ ВСЕГДА использовать тематические классы (btn-primary, btn-info, btn-tomorrow, etc.)
   - ✅ Все цвета должны управляться через систему тем в `/client/src/lib/design-system.css`
   - ✅ При добавлении новых цветов создавать соответствующие CSS переменные и классы

2. **ТЕКСТОВЫЕ ЭЛЕМЕНТЫ:**
   - ❌ НИКОГДА не добавлять хардкод текста на русском или других языках
   - ✅ ВСЕГДА использовать ключи переводов через функции t(), adminT(), shopT()
   - ✅ При добавлении новых текстов создавать переводы на все 4 языка:
     - Русский (ru) - основной язык
     - Английский (en) 
     - Иврит (he) - RTL поддержка
     - Арабский (ar) - RTL поддержка
   - ✅ Добавлять переводы в соответствующие файлы: `/client/src/locales/{lang}/admin.json`, `common.json`, `shop.json`

3. **ПОСЛЕДОВАТЕЛЬНОСТЬ ДЕЙСТВИЙ:**
   - При добавлении новой кнопки: создать CSS класс → добавить переводы → использовать в компоненте
   - При изменении UI: проверить RTL совместимость для иврита и арабского
   - При добавлении функционала: убедиться в многоязычной поддержке

## Changelog

- January 17, 2025: ✅ VPS СЕРВЕР: ИСПРАВЛЕНЫ КРИТИЧЕСКИЕ БАГИ СКАНЕРА ШТРИХ-КОДОВ
  - ПРОБЛЕМА #1: Бесконечные сообщения "вес товара изменен" при сканировании существующих товаров
  - ПРОБЛЕМА #2: Диалог добавления товара не закрывается после нажатия кнопки "Добавить"
  - КОРНЕВАЯ ПРИЧИНА: Функция handleBarcodeDetected вызывалась многократно из-за неостановленного сканера
  - ИСПРАВЛЕНИЯ:
    - Немедленная остановка сканирования при обнаружении штрих-кода (setIsScanning(false) в начале функции)
    - Обнуление сканера (codeReaderRef.current = null) для предотвращения повторных вызовов
    - Увеличена защита от дублирования с 2 до 3 секунд
    - Тройное закрытие диалога добавления товара с разными интервалами (50ms, 150ms, 200ms)
    - Задержки для toast уведомлений (100ms) для предотвращения конфликтов состояния
    - Исправлены все TypeScript ошибки (типы параметров, статусы камеры)
  - РЕЗУЛЬТАТ: Система сканирования теперь работает корректно - одно сканирование = одно действие
...
- January 17, 2025: ✅ СЕРВЕР: СОЗДАНО РЕШЕНИЕ ДЛЯ ОТКАТА ПРОБЛЕМНОЙ МИГРАЦИИ ШТРИХ-КОДОВ
  ...
  - РЕШЕНИЕ СОЗДАНО: migrations/rollback-barcode-system.sql для безопасного отката избыточных изменений
  - ДОКУМЕНТАЦИЯ: SERVER-BARCODE-MIGRATION-FIX.md с подробными инструкциями исправления
  - РЕЗУЛЬТАТ: После отката сервер будет работать идентично Replit со всей функциональностью штрих-кодов
- January 16, 2025: ✅ СИСТЕМА СКАНИРОВАНИЯ ШТРИХ-КОДОВ ПОЛНОСТЬЮ ФУНКЦИОНАЛЬНА
  - УСПЕШНОЕ ЗАВЕРШЕНИЕ: Камерное сканирование штрих-кодов работает от начала до конца
  - ТЕХНИЧЕСКОЕ РЕШЕНИЕ: Исправлена логика обработки результатов ZXing библиотеки
    - Устранена ошибка "result.getFormat is not a function" через безопасную проверку методов
    - Исправлена логика callback: теперь проверяется `result && !error` вместо только `result`
    - Добавлена детальная диагностика поиска товаров для мобильной отладки
    - Исправлена проблема бесконечных toast-сообщений через setTimeout
  - ДИАГНОСТИЧЕСКАЯ СИСТЕМА: Создана полная система отладки для мобильных устройств
    - Пошаговое отслеживание: инициализация → распознавание → парсинг → поиск → обработка
    - Показ всех товаров с штрих-кодами в базе данных
    - Детальная проверка совпадения кодов продуктов
    - Индикация статуса каждого этапа обработки
  - ФУНКЦИОНАЛЬНОСТЬ: Система корректно обрабатывает израильские штрих-коды
    - Распознавание формата "2025874002804" → код "025874", вес "280.4г"
    - Поиск товара "Абжерка" по коду "025874" в базе данных
    - Автоматическое обновление веса в заказе или добавление нового товара
    - Корректное закрытие сканера после успешной обработки
  - РЕЗУЛЬТАТ: Полностью рабочая система сканирования для весового оборудования
- January 16, 2025: ✅ КРИТИЧЕСКАЯ ОШИБКА ТЕСТОВОЙ КНОПКИ ИСПРАВЛЕНА
  - ПРОБЛЕМА РЕШЕНА: TypeError "codeReaderRef.current.reset is not a function" при тестировании сканера
  - РЕШЕНИЕ: Убрана несуществующая функция reset() из ZXing библиотеки
  - ТЕХНИЧЕСКОЕ ИСПРАВЛЕНИЕ:
    - Удалена попытка вызова codeReaderRef.current.reset() в функции handleBarcodeDetected
    - Упрощена логика остановки сканера в функции stopScanning
    - Сканер теперь корректно обнуляется простым присваиванием null
  - РЕЗУЛЬТАТ: Тестовая кнопка "🧪 Тест" теперь работает без ошибок
  - СТАТУС: Базовая функциональность восстановлена, готова к тестированию камеры

- January 15, 2025: ✅ МОБИЛЬНАЯ ДИАГНОСТИКА СКАНЕРА ШТРИХ-КОДОВ РЕАЛИЗОВАНА
  - ПРОБЛЕМА РЕШЕНА: Невозможность диагностики сканера на мобильных устройствах без доступа к консоли
  - РЕШЕНИЕ: Интегрированная система диагностики прямо в интерфейсе сканера
  - КЛЮЧЕВЫЕ ВОЗМОЖНОСТИ:
    - Панель отладки с временными метками показывает каждый этап инициализации
    - Индикатор статуса камеры с цветовой кодировкой (серый/желтый/зеленый/красный)
    - Детальные сообщения об ошибках на русском языке для пользователей
    - Кнопка "Повторить" для повторного запуска сканера после ошибки
  - ЭТАПЫ ДИАГНОСТИКИ:
    - 🔍 Инициализация сканера
    - 📱 Запрос доступа к камере
    - ✅ Камера получена (задняя/передняя)
    - 🎥 Поток подключен к видео элементу
    - 📐 Метаданные видео загружены
    - ▶️ Воспроизведение видео запущено
    - 🔍 Детектор штрих-кодов активен
    - 🎯 Сканер полностью готов
  - ОБРАБОТКА ОШИБОК:
    - Специфические сообщения для каждого типа ошибки камеры
    - Автоматическое переключение на альтернативные методы сканирования
    - Детальное логирование для технической диагностики
  - ПОЛЬЗОВАТЕЛЬСКИЙ ИНТЕРФЕЙС:
    - Мобильно-оптимизированная панель диагностики
    - Максимум 5 последних сообщений для экономии места
    - Индикатор состояния камеры с понятными текстовыми описаниями
    - Корректная обработка ресурсов камеры при закрытии
  - РЕЗУЛЬТАТ: Полная диагностика сканера доступна на мобильных устройствах
- January 15, 2025: ✅ ДОБАВЛЕНЫ ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА РАЗРАБОТКИ
  - Создан раздел "Core Development Guidelines" с требованиями по цветам и текстам
  - Установлены строгие правила использования тематических CSS классов
  - Определены требования многоязычной поддержки для всех текстовых элементов
  - Добавлены инструкции по RTL поддержке для иврита и арабского языков
- January 14, 2025: ✅ ПОЛНОСТЬЮ АВТОМАТИЧЕСКАЯ СИСТЕМА ОБНОВЛЕНИЙ БЕЗ УВЕДОМЛЕНИЙ - ЗАВЕРШЕНА И ПРОТЕСТИРОВАНА
  - РЕШЕНИЕ ПРОБЛЕМЫ: Создана идеальная система автоматических обновлений без участия пользователя
  - КАРДИНАЛЬНЫЕ ИЗМЕНЕНИЯ:
    - CacheBuster: При обнаружении нового хеша приложение автоматически обновляется без уведомлений
    - IOSCacheBuster: Специальная автоматическая очистка кеша для iOS устройств
    - Убраны все UI компоненты уведомлений и alert-сообщения
    - Убраны кнопки "Обновить" и "Позже"
    - Система работает полностью в фоновом режиме
  - ПОЛЬЗОВАТЕЛЬСКИЙ ОПЫТ: 
    - Никаких действий от пользователя не требуется
    - Обновления происходят прозрачно каждые 30 секунд
    - iOS устройства получают агрессивную очистку кеша каждые 10 секунд
    - Автоматическая перезагрузка при обнаружении изменений
  - ТЕХНИЧЕСКАЯ РЕАЛИЗАЦИЯ:
    - performAutoUpdate() - автоматическое обновление для обычных устройств
    - performIOSAutoUpdate() - специальная функция для iOS с агрессивной очисткой
    - Стабильный тестовый режим: тестовый хеш активен 5 минут для надежного тестирования
    - Сохранение обработанных хешей в localStorage для предотвращения дублирования
    - Service Worker очистка и полная перезагрузка страницы
  - ТЕСТИРОВАНИЕ:
    - Создана тестовая система с endpoint /api/version?test=notification
    - AdminCacheBuster кнопка "🧪 Тест автообновления" для проверки системы
    - Подтверждена работа автоматической перезагрузки после обнаружения изменений
    - Убраны временные alert-уведомления после успешного тестирования
  - РЕЗУЛЬТАТ: Полностью прозрачная, протестированная система обновлений без участия пользователя
